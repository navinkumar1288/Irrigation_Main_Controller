...
bool ModemBase::configureHotspot(const String &ssid, const String &password) {
  if (!modemReady) {
    Serial.println("[Modem] ❌ Hotspot config failed - modem not ready");
    return false;
  }

  Serial.println("[Modem] Configuring WiFi hotspot...");
  Serial.println("[Modem] SSID: " + ssid);

  // Step 1: Enable WiFi functionality
  Serial.println("[Modem] → Enabling WiFi...");
  String resp = sendCommand("AT+QWIFI=1", 5000);
  if (resp.indexOf("OK") < 0) {
    Serial.println("[Modem] ⚠ WiFi enable warning (may already be enabled)");
    // Continue anyway - WiFi may already be enabled
  }
  delay(1000);

  // Step 2: Configure authentication mode (WPA2-PSK)
  Serial.println("[Modem] → Setting auth mode...");
  resp = sendCommand("AT+QWAPCFG=\"authmode\"," + String(MODEM_HOTSPOT_AUTH_MODE), 2000);
  if (resp.indexOf("OK") < 0) {
    Serial.println("[Modem] ❌ Auth mode config failed");
    return false;
  }

  // Step 3: Configure maximum stations
  Serial.println("[Modem] → Setting max stations...");
  resp = sendCommand("AT+QWAPCFG=\"max_sta_num\"," + String(MODEM_HOTSPOT_MAX_STA), 2000);
  if (resp.indexOf("OK") < 0) {
    Serial.println("[Modem] ❌ Max stations config failed");
    return false;
  }

  // Step 4: Configure WiFi channel
  Serial.println("[Modem] → Setting channel...");
  resp = sendCommand("AT+QWAPCFG=\"channel\"," + String(MODEM_HOTSPOT_CHANNEL), 2000);
  if (resp.indexOf("OK") < 0) {
    Serial.println("[Modem] ❌ Channel config failed");
    return false;
  }

  // Step 5: Configure WiFi AP parameters
  // Format: AT+QWAP=1,"<ssid>","<password>",<band>,<encrypt>,<dhcp>
  Serial.println("[Modem] → Configuring AP parameters...");
  String apCmd = "AT+QWAP=1,\"" + ssid + "\",\"" + password + "\"," +
                 String(MODEM_HOTSPOT_BAND) + "," +
                 String(MODEM_HOTSPOT_ENCRYPT) + "," +
                 String(MODEM_HOTSPOT_DHCP);
  resp = sendCommand(apCmd, 5000);
  if (resp.indexOf("OK") < 0) {
    Serial.println("[Modem] ❌ AP configuration failed");
    return false;
  }

  Serial.println("[Modem] ✓ Hotspot configured successfully");
  return true;
}

bool ModemBase::startHotspot() {
  if (!modemReady) {
    Serial.println("[Modem] ❌ Hotspot start failed - modem not ready");
    return false;
  }

  Serial.println("[Modem] Starting WiFi hotspot...");

  // Start hotspot: AT+QWAP=2
  String resp = sendCommand("AT+QWAP=2", 10000);

  if (resp.indexOf("OK") >= 0) {
    Serial.println("[Modem] ✓ WiFi hotspot started");
    delay(2000);  // Wait for hotspot to fully start

    // Check hotspot status
    String status = getHotspotStatus();
    Serial.println("[Modem] Hotspot status: " + status);

    return true;
  } else {
    Serial.println("[Modem] ❌ Hotspot start failed");
    return false;
  }
}

bool ModemBase::stopHotspot() {
  if (!modemReady) {
    Serial.println("[Modem] ❌ Hotspot stop failed - modem not ready");
    return false;
  }

  Serial.println("[Modem] Stopping WiFi hotspot...");

  // Stop hotspot: AT+QWAP=0
  String resp = sendCommand("AT+QWAP=0", 5000);

  if (resp.indexOf("OK") >= 0) {
    Serial.println("[Modem] ✓ WiFi hotspot stopped");
    return true;
  } else {
    Serial.println("[Modem] ❌ Hotspot stop failed");
    return false;
  }
}

String ModemBase::getHotspotStatus() {
  if (!modemReady) {
    return "Modem not ready";
  }

  // Query hotspot status: AT+QWAP?
  String resp = sendCommand("AT+QWAP?", 3000);

  // Parse response to extract status
  if (resp.indexOf("+QWAP:") >= 0) {
    // Extract the status line
    int start = resp.indexOf("+QWAP:");
